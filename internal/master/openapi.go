// openapi.go embeds the Swagger UI HTML template and serves the OpenAPI spec
// for the master REST API documentation endpoint.
package master

import "fmt"

// SwaggerUIHTML is the HTML template for embedding Swagger UI in the API documentation endpoint.
// The %s placeholder is replaced with the JSON-formatted OpenAPI specification generated by GenerateOpenAPISpec().
// This allows the master to serve interactive API documentation at the /docs endpoint, enabling users to explore
// and test the REST API directly from their browsers. The template includes links to the Swagger UI CSS and JS
// from a CDN, and initializes the UI with the provided OpenAPI spec on page load.
...
//
// Dynamically constructs a complete OpenAPI specification document that describes all
// REST endpoints, request/response schemas, authentication method (API key), and server
// information. The spec is embedded with the current API version and server base URL
// (derived from the master's configuration). This enables API documentation, client
// SDK generation, and interactive testing via Swagger UI. The specification includes
// detailed path definitions, request bodies, response schemas, and security requirements.
//
	...
	"properties": {
	  ...
	  "MasterInfo": {
		...
		"properties": {
		  ...
		  "ver": {"type": "string", "description": "Version"},
