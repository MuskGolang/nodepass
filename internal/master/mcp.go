// mcp.go implements the Model Context Protocol (MCP) HTTP endpoint for the
// Master API. It routes JSON-RPC 2.0 requests to the appropriate handler and
// provides tool schema definitions, tool execution logic, and shared
// response/error writers used by all MCP sub-handlers.
package master
...
// HandleMCP is the single HTTP entry point for all MCP JSON-RPC 2.0 requests.
// It validates the envelope and dispatches to the appropriate method handler.
//
// Only accepts POST requests. Decodes the JSON-RPC 2.0 request envelope and validates
// that the jsonrpc field is "2.0". Routes to handlers based on the method field:
//   - "initialize": Performs the MCP handshake
//   - "tools/list": Returns available tools and their input schemas
//   - "tools/call": Executes a tool and returns the result
//
// Unknown methods receive a -32601 Method Not Found error. Parse errors receive -32700.
// All responses are JSON-RPC 2.0 compatible with either a result or error field.
...
// HandleMCPInitialize responds to the MCP "initialize" handshake with server
// capabilities and version information.
//
// Returns a JSON-RPC response with the MCP protocol version, server name ("Master"),
// server version (binary version), and advertised capabilities (tools). This allows
// MCP clients to discover what features the master supports and negotiate compatibility.
...
//
// Each tool includes a description and input schema (JSON Schema) with required fields.
// Common parameters like id, type, tunnel_port, etc. are defined once and reused.
func (m *Master) HandleMCPToolsList(w http.ResponseWriter, req MCPRequest) {
	commonParams := map[string]map[string]any{
		...
		"type": {
			"type":        "string",
			"description": "Instance type: server or client",
		...
		"pool": {
			"type":        "string",
			"description": "Connection pool type: 0=TCP, 1=QUIC, 2=WebSocket, 3=HTTP/2",
			"enum":        []string{"0", "1", "2", "3"},
		},
		"dial": {
			"type":        "string",
	...
	tools := []map[string]any{
		...
		},
		{
			...
			"description": "Create a new instance with structured configuration",
			"inputSchema": map[string]any{
				"type": "object",
				"properties": map[string]any{
				...
				"required": []string{"type", "tunnel_port", "target_port"},
			},
		},
		{
		...
		},
		{
			...
			"description": "Set instance connection pool settings (mode, pool size limits)",
			"inputSchema": map[string]any{
				"type": "object",
				"properties": map[string]any{
					"id":   commonParams["id"],
...
//
// Decodes the tool call parameters (name and arguments) and dispatches to the appropriate
// handler for one of 18 available tools. Each tool handler validates required parameters,
// performs the requested operation, and returns a result with a content field (human-readable
// text) and optional data fields (structured results like instances or config). Errors are
// returned as JSON-RPC error responses with code -32602 (Invalid params) and a descriptive message.
//
// Tool categories:
...
func (m *Master) HandleMCPToolsCall(w http.ResponseWriter, req MCPRequest) {
	...
	case "create_instance":
		instanceType, _ := params.Arguments["type"].(string)
		if instanceType != "server" && instanceType != "client" {
			m.WriteMCPError(w, req.ID, -32602, "Invalid params", "type must be 'server' or 'client'")
			return
		}

		tunnelAddr, _ := params.Arguments["tunnel_address"].(string)
		if tunnelAddr == "" {
			tunnelAddr = map[string]string{"server": "", "client": ""}[instanceType]
		}
		...
		if password, ok := params.Arguments["password"].(string); ok && password != "" {
			instanceURL = fmt.Sprintf("%s://%s@%s:%s/%s", instanceType, password, tunnelAddr, tunnelPort, targetPath)
		} else {
			instanceURL = fmt.Sprintf("%s://%s:%s/%s", instanceType, tunnelAddr, tunnelPort, targetPath)
		}
		...
		skipKeys := map[string]bool{"alias": true, "type": true, "tunnel_address": true, "tunnel_port": true, "target_address": true, "target_port": true, "targets": true, "password": true}
	...
	case "set_instance_connection":
		...
		if pool, ok := params.Arguments["pool"].(string); ok && pool != "" {
			updates["pool"] = pool
		}
...
//
// Constructs a response envelope with JSONRPC="2.0", the request ID, and the result.
// Sets Content-Type to application/json and HTTP 200 status. Encodes the response as JSON.
//
// Parameters:
...
// WriteMCPError writes a JSON-RPC 2.0 error response to w using the given
// error code, message, and optional extra data.
//
// Constructs an error response envelope with JSONRPC="2.0", the request ID, and an error
// object containing code, message, and optional data. Standard JSON-RPC error codes:
//   - -32700: Parse error
...
//
// Sets Content-Type to application/json and HTTP 200 status (per JSON-RPC spec).
